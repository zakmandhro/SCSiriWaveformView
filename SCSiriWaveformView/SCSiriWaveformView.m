...
- (void)setup
{
    ...
    CGFloat step = 1.0 / self.numberOfWaves;
    NSMutableArray *generatedColors = [[NSMutableArray alloc]initWithCapacity:self.numberOfWaves];
    
    for (int i = self.numberOfWaves; i > 0; i--) {
        UIColor* color = [UIColor colorWithRed:0 green:0 blue:i*step alpha: i*step];
        [generatedColors addObject:color];
    }
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i = 0; i < self.numberOfWaves; i++) {
        ...
        [[color colorWithAlphaComponent:multiplier * CGColorGetAlpha(self.waveColor.CGColor)] set];
